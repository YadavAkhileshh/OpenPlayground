...
function render(){
 ...
 filteredRecipes.forEach(r=>{
  ...
  const formatBadge = r.requiresPNG ? '<div class="format-badge">🖼️ PNG</div>' : '';
...
async function checkImageFormat(id) {
  ...
  try {
    ...
    let message = `📷 Image Format Analysis for "${recipe.title}"\n\n`;
    ...
    if (formatInfo.formatMismatch) {
      message += `⚠️ FORMAT MISMATCH DETECTED!\n`;
    ...
    if (originalUrl.includes('unsplash.com')) {
      message += `ℹ️ Note: Unsplash always serves images in JPEG format, regardless of the URL extension.\n`;
...
function openRecipe(id){
 ...
 const formatWarning = r.requiresPNG && r.img && r.img.includes('unsplash.com') ? 
   '<div class="format-warning">⚠️ <strong>Format Notice:</strong> This recipe requires PNG format for transparency support. External image served as JPEG. Using local PNG fallback for optimal display.</div>' : '';
...
function openAdd(){
 ...
 document.getElementById('modal').innerHTML=`<div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="add-title" tabindex="-1">
   ...
   <div class="format-info">
     <strong>📝 Image Format Guidelines:</strong><br>
     • <strong>PNG Format:</strong> Required for recipes needing transparency support<br>
     • <strong>External Sources:</strong> Unsplash/Picsum serve JPEG regardless of URL extension<br>
     • <strong>Supported:</strong> JPEG, PNG, WebP, SVG • <strong>Max Size:</strong> 2MB<br>
     • <strong>Recommendation:</strong> Use local PNG files for transparency requirements
   </div>
   ...
   <div style="margin:10px 0;">
     <label style="display:flex;align-items:center;gap:8px;">
       ...
       <span>🖼️ This recipe requires PNG format (transparency/alpha channel support)</span>
     </label>
   </div>
  ...
  fileInput.addEventListener('change', async (e) => {
    ...
    uploadInfo.textContent = `${file.name} • ${(file.size/1024).toFixed(0)} KB • ${file.type}`;
...
function addRecipe(){
 ...
 if (requiresPNG && (imgUrlInput.includes('unsplash.com') || imgUrlInput.includes('picsum.photos'))) {
   const proceed = confirm('⚠️ Warning: You\'ve marked this recipe as requiring PNG format, but you\'re using an external image source that typically serves JPEG. This may cause format inconsistency. Continue anyway?');
   if (!proceed) return;
 }
 ...
 if (window.__uploadedImageDataURL) {
   ...
   if (requiresPNG && finalImgType !== 'image/png') {
     const proceed = confirm(`⚠️ PNG required but uploaded file type is ${finalImgType}. Transparency may not be supported. Continue?`);
     if (!proceed) return;
   }
 }
 ...
 alert(`✅ Recipe "${title}" added successfully!${formatNote}`);
}
